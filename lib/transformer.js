...
export async function transformCue(japaneseText, options = {}) {
  const { provider = 'groq', apiKey, model, mode = 'llm' } = options;
  ...
  try {
    if (config.isAnthropic) {
      return await callAnthropic(japaneseText, apiKey, selectedModel);
    }
    if (config.isGemini) {
      return await callGemini(japaneseText, apiKey, selectedModel);
    }
    return await callOpenAICompatible(japaneseText, config, apiKey, selectedModel);
  } catch (err) {
