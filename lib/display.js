...
const ProperSubsDisplay = (() => {
  ...
  function loadSettings() {
    chrome.storage.sync.get({
      ...
      // Apply custom particle colors if configured
      if (settings.customParticleColors) {
        Object.assign(PARTICLE_COLORS, settings.customParticleColors);
      }
    });
  }
  ...
  function renderDual(element, structuredText, originalText) {
    const structuredHtml = colorizeParticles(structuredText);
    element.innerHTML =
