...
const ProperSubsDisplay = (() => {
  ...
  // ── Passthrough render ─────────────────────────────────────────────────
  // Shows the raw captured subtitle on the video frame. Detects video
  // frame size, calculates overlay width and position from the video's
  // bounding rect, places it in the lower portion above player controls.
  ...
  function renderPassthrough(subtitleElement, rawText) {
    ...
    // Get video frame dimensions and screen position
    ...
    if (!passthroughElement) {
      ...
      document.body.appendChild(passthroughElement);
    }

    passthroughElement.textContent = rawText;

    // Position: fixed to viewport, calculated from video frame rect
    // Width: 90% of video frame width, centered
    // Bottom: 48px above the bottom of the video frame (above controls)
    const subWidth = rect.width * 0.9;
    const subLeft = rect.left + (rect.width - subWidth) / 2;
    const subTop = rect.bottom - 48;

    passthroughElement.style.cssText =
      `position:fixed;` +
      `left:${subLeft}px;` +
      `top:${subTop}px;` +
      `width:${subWidth}px;` +
