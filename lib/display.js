...
const ProperSubsDisplay = (() => {
  ...
  // ── Passthrough render ─────────────────────────────────────────────────
  // Shows the raw captured subtitle on the video frame to prove capture
  // is working. Positioned at the natural lower end of the video, above
  // player controls. Dark gray background, yellow text matching theme.
  let passthroughElement = null;

  function renderPassthrough(subtitleElement, rawText) {
    if (!rawText) return;

    // Find the video element to position the overlay correctly
    const video = document.querySelector('video');
    if (!video) return;

    if (!passthroughElement) {
      passthroughElement = document.createElement('div');
      passthroughElement.id = 'propersubs-passthrough';
      passthroughElement.className = 'propersubs-passthrough';

      // Insert into the video's parent so it's relative to the player
      const parent = video.parentElement || document.body;
      parent.style.position = parent.style.position || 'relative';
      parent.appendChild(passthroughElement);
    }

    passthroughElement.textContent = rawText;

    // Position above typical player controls (~48px from bottom)
    const videoRect = video.getBoundingClientRect();
    const parentRect = (video.parentElement || document.body).getBoundingClientRect();

    passthroughElement.style.position = 'absolute';
    passthroughElement.style.bottom = '48px';
    passthroughElement.style.left = '0';
    passthroughElement.style.width = '100%';
    passthroughElement.style.textAlign = 'center';
    passthroughElement.style.pointerEvents = 'none';
    passthroughElement.style.zIndex = '2147483646';
  }
  ...
  return { render, renderPassthrough, clear, colorizeParticles };
})();
